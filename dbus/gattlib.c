...
static const char *m_dbus_error_unknown_object =
    "GDBus.Error:org.freedesktop.DBus.Error.UnknownObject";

gboolean on_handle_device_property_change(
    OrgBluezGattCharacteristic1 *object, GVariant *arg_changed_properties,
    const gchar *const *arg_invalidated_properties, gpointer user_data) {
            ...
            if (strcmp(key, "Connected") == 0) {
                if (!g_variant_get_boolean(value)) {
                    ...
                    if (gattlib_has_valid_handler(&connection->disconnection)) {
                        gattlib_call_disconnection_handler(
...
void get_device_path_from_mac_with_adapter(OrgBluezAdapter1 *adapter,
                                           const char *mac_address,
                                           char *object_path,
                                           size_t object_path_len) {
    char device_address_str[20 + 1];
    const char *adapter_path = g_dbus_proxy_get_object_path(
        (GDBusProxy *)ORG_BLUEZ_ADAPTER1_PROXY(adapter));
    ...
    snprintf(object_path, object_path_len, "%s/dev_%s", adapter_path,
             device_address_str);
}
...
void get_device_path_from_mac(const char *adapter_name, const char *mac_address,
    ...
    // Generate object path like: /org/bluez/hci0/dev_DA_94_40_95_E0_87
    snprintf(object_path, object_path_len, "/org/bluez/%s/dev_%s", adapter,
             device_address_str);
}
 ...
 * @param dst_type	Set LE address type (either BDADDR_LE_PUBLIC or
 * BDADDR_LE_RANDOM)
 * @param sec_level	Set security level (either BT_IO_SEC_LOW,
 * BT_IO_SEC_MEDIUM, BT_IO_SEC_HIGH)
...
gatt_connection_t *gattlib_connect(void *adapter, const char *dst,
    ...
    get_device_path_from_mac(adapter_name, dst, object_path,
                             sizeof(object_path));
    ...
    g_signal_connect(device, "g-properties-changed",
                     G_CALLBACK(on_handle_device_property_change), connection);

    error = NULL;
    org_bluez_device1_call_connect_sync(device, NULL, &error);
    if (error) {
        if (strncmp(error->message, m_dbus_error_unknown_object,
                    strlen(m_dbus_error_unknown_object)) == 0) {
            // You might have this error if the computer has not scanned or has
            // not already had pairing information about the targetted device.
            fprintf(stderr, "Device '%s' cannot be found\n", dst);
        } else {
            fprintf(stderr, "Device connected error (device:%s): %s\n",
                    conn_context->device_object_path, error->message);
        }
    ...
    // Wait for the property 'UUIDs' to be changed. We assume
    // 'org.bluez.GattService1 and 'org.bluez.GattCharacteristic1' to be
    // advertised at that moment.
    conn_context->connection_loop = g_main_loop_new(NULL, 0);

    conn_context->connection_timeout = g_timeout_add_seconds(
        CONNECT_TIMEOUT, stop_scan_func, conn_context->connection_loop);
    ...
    conn_context->dbus_objects =
        g_dbus_object_manager_get_objects(device_manager);
...
// Bluez was using org.bluez.Device1.GattServices until 5.37 to expose the list
// of available GATT Services
#if BLUEZ_VERSION < BLUEZ_VERSIONS(5, 38)
int gattlib_discover_primary(gatt_connection_t *connection,
    ...
    const gchar *const *service_strs =
        org_bluez_device1_get_gatt_services(device);
    ...
    gattlib_primary_service_t *primary_services =
        malloc(count_max * sizeof(gattlib_primary_service_t));
    ...
    for (service_str = service_strs; *service_str != NULL; service_str++) {
        ...
        if (org_bluez_gatt_service1_get_primary(service_proxy)) {
            ...
            gattlib_string_to_uuid(
                ...
                MAX_LEN_UUID_STR + 1, &primary_services[count].uuid);
            count++;
        }
...
int gattlib_discover_primary(gatt_connection_t *connection,
    ...
    gattlib_context_t *conn_context = connection->context;
    GDBusObjectManager *device_manager =
        get_device_manager_from_adapter(conn_context->adapter);
    OrgBluezDevice1 *device = conn_context->device;
    const gchar *const *service_str;
    GError *error = NULL;
    ...
    for (l = conn_context->dbus_objects; l != NULL; l = l->next) {
        ...
        GDBusInterface *interface = g_dbus_object_manager_get_interface(
            device_manager, object_path, "org.bluez.GattService1");
        if (!interface) {
            continue;
        }
        ...
        OrgBluezGattService1 *service_proxy =
            org_bluez_gatt_service1_proxy_new_for_bus_sync(
                G_BUS_TYPE_SYSTEM, G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_NONE,
                "org.bluez", object_path, NULL, &error);
        if (service_proxy == NULL) {
            if (error) {
                fprintf(stderr, "Failed to open service '%s': %s\n",
        ...
        if (strcmp(conn_context->device_object_path,
                   org_bluez_gatt_service1_get_device(service_proxy))) {
            g_object_unref(service_proxy);
            continue;
        }
        ...
        if (org_bluez_gatt_service1_get_primary(service_proxy)) {
            ...
            for (GList *m = l; m != NULL; m = m->next) {
                ...
                interface = g_dbus_object_manager_get_interface(
                    device_manager, characteristic_path,
                    "org.bluez.GattCharacteristic1");
                if (!interface) {
                    continue;
                } else if (strncmp(object_path, characteristic_path,
                                   strlen(object_path)) != 0) {
                    g_object_unref(interface);
                    continue;
                } else {
                    int char_handle;
                    ...
                    // '/org/bluez/hci0/dev_DE_79_A2_A1_E9_FA/service0024/char0029'.
                    // We convert the last 4 hex characters into the handle
                    sscanf(
                        characteristic_path + strlen(characteristic_path) - 4,
                        "%x", &char_handle);

                    if (primary_services[count].attr_handle_start == 0) {
                        primary_services[count].attr_handle_start = char_handle;
                    } else {
                        primary_services[count].attr_handle_start =
                            MIN(primary_services[count].attr_handle_start,
                                char_handle);
                    }
                    ...
                    if (primary_services[count].attr_handle_end == 0) {
                        ...
                        primary_services[count].attr_handle_end =
                            MAX(primary_services[count].attr_handle_end,
                                char_handle);
                    }
                }
            }
...
int gattlib_discover_char_range(gatt_connection_t *connection, int start,
    ...
    for (service_str = service_strs; *service_str != NULL; service_str++) {
        ...
        characteristic_strs =
            org_bluez_gatt_service1_get_characteristics(service_proxy);
        if (characteristic_strs == NULL) {
            g_object_unref(service_proxy);
            continue;
        }

        for (characteristic_str = *characteristic_strs;
             characteristic_str != NULL; characteristic_str++) {
            // Object path is in the form
            // '/org/bluez/hci0/dev_DE_79_A2_A1_E9_FA/service0024/char0029'. We
            // convert the last 4 hex characters into the handle
            sscanf(characteristic_str + strlen(characteristic_str) - 4, "%x",
                   &handle);
    ...
    gattlib_characteristic_t *characteristic_list =
        malloc(count_max * sizeof(gattlib_characteristic_t));
    ...
    for (service_str = service_strs; *service_str != NULL; service_str++) {
        ...
        for (characteristic_str = *characteristic_strs;
            ...
            OrgBluezGattCharacteristic1 *characteristic_proxy =
                org_bluez_gatt_characteristic1_proxy_new_for_bus_sync(
                    G_BUS_TYPE_SYSTEM, G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_NONE,
                    "org.bluez", characteristic_str, NULL, &error);
            if (characteristic_proxy == NULL) {
                ...
                for (; *flags != NULL; flags++) {
                    if (strcmp(*flags, "broadcast") == 0) {
                        characteristic_list[count].properties |=
                ...
                gattlib_string_to_uuid(org_bluez_gatt_characteristic1_get_uuid(
                                           characteristic_proxy),
                                       MAX_LEN_UUID_STR + 1,
...
#else
static void add_characteristics_from_service(
    gattlib_context_t *conn_context, GDBusObjectManager *device_manager,
    const char *service_object_path, int start, int end,
    ...
    for (GList *l = conn_context->dbus_objects; l != NULL; l = l->next) {
        ...
        if (strcmp(org_bluez_gatt_characteristic1_get_service(characteristic),
                   service_object_path)) {
            ...
            const gchar *const *flags =
                org_bluez_gatt_characteristic1_get_flags(characteristic);
            for (; *flags != NULL; flags++) {
                if (strcmp(*flags, "broadcast") == 0) {
                    characteristic_list[*count].properties |=
                        GATTLIB_CHARACTERISTIC_BROADCAST;
            ...
            gattlib_string_to_uuid(
                ...
                MAX_LEN_UUID_STR + 1, &characteristic_list[*count].uuid);
            *count = *count + 1;
        }
...
int gattlib_discover_char_range(gatt_connection_t *connection, int start,
    ...
    // Count the maximum number of characteristic to allocate the array (we
    // count all the characterstic for all devices)
    int count_max = 0, count = 0;
    for (l = conn_context->dbus_objects; l != NULL; l = l->next) {
        GDBusObject *object = l->data;
        const char *object_path =
    ...
    for (l = conn_context->dbus_objects; l != NULL; l = l->next) {
        ...
        add_characteristics_from_service(conn_context, device_manager,
                                         object_path, start, end,
                                         characteristic_list, &count);
        g_object_unref(service_proxy);
    }
...
int gattlib_discover_char(gatt_connection_t *connection,
                          gattlib_characteristic_t **characteristics,
                          int *characteristics_count) {
    return gattlib_discover_char_range(connection, 0x00, 0xFF, characteristics,
                                       characteristics_count);
}

int gattlib_discover_desc_range(gatt_connection_t *connection, int start,
                                int end, gattlib_descriptor_t **descriptors,
                                int *descriptor_count) {
    return GATTLIB_NOT_SUPPORTED;
}

int gattlib_discover_desc(gatt_connection_t *connection,
                          gattlib_descriptor_t **descriptors,
                          int *descriptor_count) {
    return GATTLIB_NOT_SUPPORTED;
}

int get_bluez_device_from_mac(struct gattlib_adapter *adapter,
                              const char *mac_address,
    ...
    if (adapter != NULL) {
        get_device_path_from_mac_with_adapter(adapter->adapter_proxy,
                                              mac_address, object_path,
                                              sizeof(object_path));
    } else {
        get_device_path_from_mac(NULL, mac_address, object_path,
                                 sizeof(object_path));
    }
...
#if 0  // Disable until https://github.com/labapart/gattlib/issues/75 is
       // resolved
...
int gattlib_get_rssi_from_mac(void *adapter, const char *mac_address,
                              int16_t *rssi) {
    OrgBluezDevice1 *bluez_device1;
    int ret;
